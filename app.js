...
function teacherQuestions(){return `<section class="card"><div class="section-title"><div><h2>Question Bank</h2><p class="muted">Only the question, options and private answer are part of the question record.</p></div><button class="btn primary" data-tab="import">Import Questions</button></div><div class="toolbar"><input id="qSearch" placeholder="Search question"></div><div id="qTable" class="table-wrap"></div></section>`;}
function renderQuestionTable(filter=''){const rows=state.teacher.questions.filter(q=>String(q.question_text||'').toLowerCase().includes(filter.toLowerCase()));return `<table><thead><tr><th>No.</th><th>Question</th><th>Status</th><th>Manage</th></tr></thead><tbody>${rows.map(q=>`<tr><td>${esc(q.source_question_number||'—')}</td><td>${esc(q.question_text)}</td><td><span class="badge good">Stored</span></td><td><button class="btn danger sm" data-delete-question="${q.id}">Delete</button></td></tr>`).join('')||`<tr><td colspan="4">${empty('No questions found.')}</td></tr>`}</tbody></table>`;}
...
function teacherImport(){return `<section class="card"><h2>Import Questions</h2><p class="muted">Upload the question PDF and, when available, a separate answer-key PDF. The system stores only the question, four options and the private correct answer.</p><div class="grid2"><label>Question file<input id="qFile" type="file" accept=".pdf,.docx,.xlsx,.xls,.csv,.txt"></label><label>Answer key (optional)<input id="aFile" type="file" accept=".pdf,.docx,.xlsx,.xls,.csv,.txt"></label></div><div class="row gap top16"><button class="btn primary" id="previewImport">Preview Questions</button><button class="btn secondary" id="clearImport">Clear</button></div><div id="importPreview" class="top16"></div></section>`;}
...
async function readFile(file){
  ...
  if(ext==='pdf'){
    ...
    if(!window.pdfjsLib.GlobalWorkerOptions) throw new Error('PDF worker is not available. Refresh the page and try again.');
    ...
    for(let pageNo=1; pageNo<=pdf.numPages; pageNo++){
      ...
      // Rebuild lines conservatively. The previous parser used a ±3pt row
      // tolerance, which merged consecutive question lines in some PDFs.
      const rows=[];
      for(const item of items){
        let best=null,bestDiff=Infinity;
        for(const r of rows){
          const d=Math.abs(r.y-item.y);
          if(d<1.15 && d<bestDiff){best=r;bestDiff=d;}
        }
        if(!best){best={y:item.y,items:[]};rows.push(best);}
        best.items.push(item);
      }
      rows.sort((a,b)=>b.y-a.y);
      for(const row of rows){
        row.items.sort((a,b)=>a.x-b.x);
        const line=row.items.map(x=>x.str).join(' ').replace(/[ \t]+/g,' ').trim();
        if(line) out+=line+'\n';
      }
      out+='\n';
    }
...
function parseAnswerKey(answerText=''){
  ...
  // Primary format used by the supplied answer-key PDF:
  // QUES 91 92 ... 100
  // ANS  B  A  ... D
  for(let i=0;i<lines.length;i++){
    ...
    if(amatch){
      const vals=(amatch[1].match(/\b[A-D]\b|\b[1-4]\b/gi)||[]).map(normalizeAnswer).filter(Boolean);
      for(let k=0;k<Math.min(nums.length,vals.length);k++) ans[nums[k]]=vals[k];
      i++;
    }
  }

  // Common one-answer-per-line formats.
  for(const m of text.matchAll(/(?:^|\n)\s*(?:Q(?:UES|UESTION)?\.?\s*)?(\d{1,4})\s*[:.)-]\s*([A-D1-4])\b/gim)){
...
function findSequentialQuestionStarts(text){
  const hits=[];
  const re=/(?:^|\n|\s)(\d{1,4})\.\s+/g;
  let last=null;
  for(const m of text.matchAll(re)){
    const n=Number(m[1]);
    if(last===null){
      // Ignore obvious page numbers / years / isolated low values before the first question.
      if(n<1) continue;
      hits.push({number:n,index:m.index + (m[0].length - m[0].replace(/^\s*/,'').length)});
      last=n;
      continue;
    }
    // Question papers are sequential. This prevents numbers embedded in question text
    // from being mistaken for new questions. Allow a small forward gap for uncommon papers.
    if(n===last+1 || (n>last+1 && n<=last+5 && hits.length<3)){
      hits.push({number:n,index:m.index});
      last=n;
    }
  }
  return hits;
...
function extractOptionsFromChunk(chunk){
  // Match uppercase MCQ labels in both `(A) text` and `A. text` forms.
  const markerRe=/(?:^|\s)\(?([A-D])\)?(?:\.|:|\)|-)\s+/g;
  const markers=[];
  for(const m of chunk.matchAll(markerRe)) markers.push({key:m[1].toUpperCase(),index:m.index + (m[0].match(/\s/)?m[0].indexOf(m[0].match(/\s/)[0]):0),end:m.index+m[0].length});
  if(markers.length<4) return null;

  // In normal MCQs the final four A-D labels are the answer choices. This also
  // avoids lowercase assertion labels such as (a), (b), etc.
  const use=markers.slice(-4);
  if(use.length!==4 || new Set(use.map(x=>x.key)).size!==4) return null;

  const question=chunk.slice(0,use[0].index).replace(/^\s+/,'').trim();
  const options=[];
  for(let i=0;i<use.length;i++){
    const from=use[i].end;
    const to=i+1<use.length?use[i+1].index:chunk.length;
    options.push(chunk.slice(from,to).replace(/\s+/g,' ').trim());
  }
  if(!question || options.length!==4 || options.some(x=>!x)) return null;
...
function parseQuestions(text,answerText=''){
  ...
  const starts=findSequentialQuestionStarts(normalized);
  const out=[];

  for(let i=0;i<starts.length;i++){
    const start=starts[i];
    const end=starts[i+1]?.index ?? normalized.length;
    const raw=normalized.slice(start.index,end).replace(/\n+/g,' ').replace(/\s+/g,' ').trim();
    const marker=raw.match(/^(\d{1,4})\.\s+(.*)$/);
    if(!marker) continue;
    const number=Number(marker[1]);
    const chunk=marker[2];
    const parsed=extractOptionsFromChunk(chunk);
    if(parsed){
      out.push({number,question:parsed.question,options:parsed.options,correct_option:ans[number]||null});
    }
  }
  return out;
}
